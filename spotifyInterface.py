...
def ensure_int(integer, default = 0):
    """Ensures the number is an integer, if not it resorts to default value.

    Args:
        integer (int): The number to check
        default (int, optional): default option if not an int. Defaults to 0.

    Returns:
        int: The number that is an int
    """    
    try:
        as_int = int(integer) #tries to convert playlist limit to int
        return as_int
    except:
        return default #if error, must me invalid, redefine
...
class spotifyMusicHandler:
    ...
    def get_from_search(self, query, query_type = "multi",offset = 0, limit = 20, num_top_results = 5):
        ...
        offset = ensure_int(offset, 0)
        limit = ensure_int(limit, 20)
        num_top_results = ensure_int(ensure_int, 5) #ensures all the values are valid ints
    ...
    def get_from_search_with_preview(self, query, query_type = "multi",offset = 0, limit = 20, num_top_results = 5):
        ...
        if not isinstance(query, str):
            return False #ensure query is string
        #no validation for other settings required, will be validated in self.get_from_search
    ...
    def get_album_tracks(self, albumID, offset = 0, limit = 300):
        ...
        offset = ensure_int(offset, 0)
        limit = ensure_int(limit, 300) #ensures all ints are valid
    ...
    def get_user_profile(self, userID, playlist_limit = 10, artist_limit = 10):
        ...
        Returns:
            dict: dict of response and duration
        """        
        playlist_limit = ensure_int(playlist_limit, 10)
        artist_limit = ensure_int(artist_limit, 10) #ensures all ints are valid
    ...
    def get_playlist(self, playlistID):
        ...
        Args:
            playlistID (str): ID of the playlist to be returned

        Returns:
            dict: dict with keys of 'results' and 'duration'
        """        
        if not isinstance(playlistID, str):
            return False #ensures playlistid is a string
        return self.get_info('/playlist/', 'GET', {"id":playlistID}) #returns the dict generated by self.get_info()
    
    def get_playlist_tracks(self, playlistID, offset = 0, limit = 100):
        """Gets the playlist tracks from the playlistID

        Args:
            playlistID (str): ID of the playlist
            offset (int, optional): Starting position of tracks to be returned. Defaults to 0.
            limit (int, optional): Limit on tracks returned. Defaults to 10.

        Returns:
            dict: the data in a dictionary, keys are 'results' and 'duration'
        """         
        if not isinstance(playlistID, str): #makes sure it is a string
            return False #otherwise exit function and say no results
        playlistID = playlistID.replace(' ', '') #removes all white spaces
        offset = ensure_int(offset, 0)
        limit = ensure_int(limit, 100) #ensures all ints are valid
        querystring = {"id":playlistID,"offset":offset,"limit":limit}
        return self.get_info('/playlist_tracks/', 'GET', querystring) #returns the dict generated by self.get_info()

if __name__ == '__main__':    
    SPOTIFY = spotifyMusicHandler('badc7baeeamsh7de16dfafae2bf6p1b8019jsn3188359ba6cf')
    results = SPOTIFY.get_playlist_tracks('37i9dQZF1DX4Wsb4d7NKfP')
    print(results)